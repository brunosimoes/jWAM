...
public class WAM extends Symbol {
	...
	public String malloc(int memorySize, int heapSize){
		...
		if(heapSize > 265 * 1024) return "ERROR: decrease heap size";

		/** 10K é um bom minimo */
		else if (heapSize < 10) return "ERROR: increase heap size";
		...
		catch(MemoryOverflow e){ return e.getMessage(); }
	...
	public void clear(){
		if(memory != null)
			for(int i = 0; i < memory.length; i++) memory[i] = null;
	}
		...
		while (P >= 0 && !fail) {
			...
			try {
				...
				switch((Integer)keys.get(s.getOp())){
					case CUT: proceed(); break;
					case GET_LEVEL: proceed(); break;
					case NECK_CUT: proceed(); break;
					case SWITCH_ON_STRUCTURE: proceed(); break;
					case SWITCH_ON_CONSTANT: proceed(); break;
					case SWITCH_ON_TERM: proceed(); break;
	
					case TRUST: trust(s.getArg()); break;
					case RETRY: retry(s.getArg()); break;
					case TRY: t_r_y(s.getArg()); break;
					case TRUST_ME: trustMe(); break;
					case RETRY_ME_ELSE: retryMeElse(s.getArg()); break;
					case TRY_ME_ELSE: tryMeElse(s.getArg()); break;
					case PROCEED: proceed(); break;
					case EXECUTE: execute(s.getArg()); break;
					case CALL: call(s.getArg()); break;
					case DEALLOCATE: deallocate(); break;
					case ALLOCATE: allocate(); break;
					case UNIFY_VOID: unifyVoid(s.getArg()); break;
					case UNIFY_CONSTANT: unifyConstant(s.getArg()); break;
					case UNIFY_LOCAL_VALUE: unifyLocalValue(s.getArg()); break;
					case UNIFY_VALUE: unifyValue(s.getArg()); break;
	
					case UNIFY_VARIABLE: proceed(); break;
	
					case SET_VOID: setVoid(s.getArg()); break;
					case SET_CONSTANT: setConstant(s.getArg()); break;
					case SET_LOCAL_VALUE: setLocalValue(s.getArg()); break;
					case SET_VALUE: setValue(s.getArg()); break;
					case SET_VARIABLE: setVariable(s.getArg()); break;
					case GET_CONSTANT: getConstant(s.getArg(), s.getArg()); break;
					case GET_LIST: getList(s.getArg()); break;
					case GET_STRUCTURE: getStructure(s.getArg(), s.getArg()); break;
					case GET_VALUE: getValue(s.getArg(), s.getArg()); break;
					case GET_VARIABLE: getVariable(s.getArg(), s.getArg()); break;
					case PUT_CONSTANT: putConstant(s.getArg(), s.getArg()); break;
					case PUT_LIST: putList(s.getArg()); break;
					case PUT_STRUCTURE: putStructure(s.getArg(), s.getArg()); break;
					case PUT_UNSAFE_VALUE: putUnsafeValue(s.getArg(), s.getArg()); break;
					case PUT_VALUE: putValue(s.getArg(), s.getArg()); break;
					case PUT_VARIABLE: putVariable(s.getArg(), s.getArg()); break;
	...
	public void putVariable(String v, String a) throws Exception {
	    switch (v.charAt(0)) {
			case 'Y': putVariableStack(v, a); break;
			case 'X': putVariableHeap(v, a); break;
			default: 
	...
	public void getStructure(String f, String Ai) throws Exception {
		...
		if (fail) backtrack();
		else registers.setP(code.getAddrNextInstruction());
	}
	...
	public void unifyValue(String Vn) throws Exception {
		...
		if(mode == READ) fail = unify(registers.getRegister(Vn).getValue(), S);
		else {
			heap.setAddress(H, registers.getRegister(Vn));
			registers.setH(H + 1);
		}
	...
	public void unifyVoid(String v) throws Exception {
		...
		if(mode == READ) registers.setS(S + n);
		else {
			for(int i = H; i <= H + n - 1; i++)
				heap.setAddress(i, i);
			registers.setH(H + n);
	...
	public void tryMeElse(String v) throws Exception {
		...
		else newB = B + stack.getValue(B) + 8;
	...
	public void t_r_y(String v) throws Exception {
		...
		else newB = B + stack.getAddress(B).getValue() + 8;
		...
		while(!(pdl.empty() || fail)){
			...
			if(d1 != d2){
				...
				if(c1.equals(REF))
					...
					switch(c2.getTag()){
						case REF: bind(d1,d2); break;
						case CON: 
							fail = ((!c1.equals(CON)) || 
							(!c1.getStringValue().equals(c2.getStringValue()))); break;
						case LIS:{
							if(!c1.equals(LIS))
								fail = true;
							else {
	...
	class Code extends Memory {
		...
		public void addSourceCode(String source){
			...
			if(tokens > 0){
				/** remove halt */
				if(lastInstruction > 0) lastInstruction --;
		...
		public String getInformation() throws InvalidInstruction {
			...
			for(int i = 0; i <= lastInstruction;){
				...
				for(int l = Util.nrDigits(i); l < t; l++)
					number += "0";
				int j = instructionSize(i);
				if(registers.getP() == i && registers.getCP() == i) s += "   P & CP   ";
				else if( registers.getP() == i) s += "     P      ";
				else if( registers.getCP() == i) s += "     CP     ";
				else s += Util.createSpaces(12);
	...
	class Heap extends Memory {
		...
		public String getInformation() throws HeapException {
			...
			for(int i = heapBottomLimit; i <= heapUpperLimit;){
				...
				if(heapPointer == i && S == i && HB == i) s += " H & S & HB ";
				else if(heapPointer == i && HB == i) s += "   H & HB   ";
				else if(heapPointer == i && S == i) s += "   H & S    ";
				else if(S == i && HB == i) s += "   S & HB   ";
				else if(heapPointer == i) s += "     H      ";
				else if(S == i) s += "     S      ";
				else if(HB == i) s += "     HB     ";
				else s += Util.createSpaces(12);
				...
				while(heapPointer != i+j && S != i+j && HB != i+j &&
						i+j <= heapUpperLimit && memory[i+j] == null) j++;
	...
	class LocalStack extends Memory {
		...
		public String getInformation() throws StackException {
			...
			for(int i = stackBottomLimit; i <= stackUpperLimit;){
				...
				if(E == i && B == i && cutPointer == i) s += " E & B & B0 ";
				else if(E == i && cutPointer == i) s += "   E & B0   ";
				else if(E == i && B == i) s += "   E & B    ";
				else if(E == i && CE == i) s += "   E & CE   ";
				else if(B == i && cutPointer == i) s += "   B & B0   ";
				else if(E == i) s += "     E      ";
				else if(CE == i) s += "     CE     ";
				else if(B == i) s += "     B      ";
				else if(cutPointer == i) s += "     B0     ";
				else s += Util.createSpaces(12);

				int j = 0;
				while(E != i+j && B != i+j && cutPointer != i+j && 
						i+j <= stackUpperLimit && memory[i+j] == null) j++;
	...
	class Trail extends Memory {
		...
		public String getInformation() throws Exception {
			...
			for(int i = trailBottomLimit; i <= registers.getTR();){//trailUpperLimit;){
				...
				if(registers.getTR() == i) s += "     TR     ";
				else s += Util.createSpaces(12);

				int j = 0;
				while(i+j <= registers.getTR() &&  memory[i+j] == null) j++;
	...
	class Registers extends Memory {
		...
		public void setRegister(String register, DataCell d){
			...
			switch (register.charAt(0)) {
				case 'A': setRegister(RA, index, d); break;
				case 'Y': memory[E + index + 2] = d; break;
				case 'X': setRegister(RX, index, d); break;
			}
		}
		...
		public DataCell getRegister(String register){
			int index = Util.getRegisterIndex(register);
			switch (register.charAt(0)) {
				case 'A': return getRegister(RA, index);
				case 'Y': return memory[E + index + 1];
				case 'X': return getRegister(RX, index);
				default: return new DataCell(Symbol.CON, "null", 0);
			}
		}
